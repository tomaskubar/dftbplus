...
module dftbp_dftb_scc
  ...
  use dftbp_dftb_coulomb, only : calcInvRPrime, invRPrime, TCoulomb, TCoulomb_init, TCoulombInput
...
#:endif
  ...
  contains
    ...
    ! (Note: this is only used in Tomas' version)
...
contains
  ...
  subroutine getExternalCharges(this, nCharge, chargeCoords, chargeQs, blurWidths)
    ...
    if (.not. allocated(this%extCharges)) then
      ...
      if (present(blurWidths)) then
        call this%extCharges%getExternalCharges(nCharge, chargeCoords, chargeQs, blurWidths=blurWidths)
      else
        call this%extCharges%getExternalCharges(nCharge, chargeCoords, chargeQs)
      end if
    end if
...
#:endif
  ...
  !> Returns the shift per atom coming from the SCC part
  !  Version for coupled-perturbed eqns: no contribution from ext. charges
  ! (Note: this is only used in Tomas' version)
