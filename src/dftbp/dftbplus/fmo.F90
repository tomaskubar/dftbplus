...
module dftbp_fmo
  ...
  use dftbp_dftb_hybridxc ! rangeseparated, only : TRangeSepFunc
  ...
  use dftbp_dftbplus_main, only : handleCoordinateChange, processPotentials
...
contains
  ...
  subroutine processGeometryPhase2(this, env, nSite, ptrsPhase1, TijOrtho, errStatus)
  ...
  ! write (*,'(14F8.4)') this%ints%overlap(1:14)
      ...
      if (allocated(this%eField%atomicSites)) then
        do iAtomicSite = 1, size(this%eField%atomicSites)
          ...
          if (this%eField%atomicOnSite(iAtomicSite)) then
            this%potential%extOnSiteAtom(iAtom,1) = this%potential%extOnSiteAtom(iAtom,1)&
                & + this%eField%atomicPotential(iAtomicSite)
          else
            this%potential%extAtom(iAtom,1) = this%potential%extAtom(iAtom,1) + this%eField%atomicPotential(iAtomicSite)
          end if
        end do
      end if
    else
...
!   end if
  ...
  ! write (*,'(14F8.4)') this%ints%hamiltonian(1:14,1)
