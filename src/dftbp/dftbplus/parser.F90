...
module dftbp_dftbplus_parser
  ...
  use dftbp_common_constants, only : Bohr__AA, boltzmann, elementSymbol, maxL, pi, shellNames, symbolToNumber
...
#:endif
  ...
  use dftbp_machinelearning, only : TMachineLearningInp
  use dftbp_machinelearning_sf, only : TMLSymmetryFunctionsInp
  use dftbp_machinelearning_nn, only : TMLNeuralNetInp, TMLNeuralNetSpeciesInp, TMLNeuralNetLayerInp,&
      & testNeuralNetInp
...
#:endif
  ...
  use dftbp_type_typegeometry, only : normalize, reduce, reduceList, setLattice, TGeometry
...
#:endif
  ...
  ! integer, allocatable :: nSymmetryFunctions(:)
    ...
    call getSelectedAtomIndices(child, char(buffer), geoIn%speciesNames, geoIn%species,&
       & input_sf%indAtomsML)
