...
module dftbp_dftbplus_mainapi
  ...
  use dftbp_dftbplus_main, only : processGeometry, buildDenseRealHam
...
contains
  ...
  subroutine getHamilOverl(env, main, hamil, overl, errStatus)
  ...
  ! call recalcGeometry(env, main)
    call buildDenseRealHam(env, main%denseDesc, main%ints, main%species, main%neighbourList,&
      & main%symNeighbourList,main%nNeighbourSK, main%iSparseStart, main%img2CentCell, main%orb, main%tPeriodic,&
      & main%tHelical, main%coord, main%electronicSolver, main%parallelKS, main%hybridXc, main%densityMatrix%deltaRhoIn,&
      & main%nNeighbourCam, main%nNeighbourCamSym, main%HSqrReal, main%SSqrReal, main%apiCallBack, main%dangerousChanges, errStatus)
    hamil(:,:) = main%HSqrReal(:,:)
    overl(:,:) = main%SSqrReal(:,:)
    ! we assume main%t2Component == .false. and main%nKPoint == 1 and main%nSpinHams == 1
...
#:endif
  ...
  subroutine getFragmentBasedHamiltonian(env, main, nSite, ptrsPhase1, TijOrtho, errStatus)
    ...
    if (main%tLatticeChanged .or. main%tCoordsChanged) then
      call processGeometryPhase2(main, env, nSite, ptrsPhase1, TijOrtho, errStatus)
      main%tLatticeChanged = .false.
      main%tCoordsChanged = .false.
    end if
