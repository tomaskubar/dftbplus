...
module dftbp_dftbplus_main
  ...
  use dftbp_derivs_perturbxderivs
! use dftbp_derivs_perturbxderivs_tk
  use dftbp_derivs_perturbxderivs_qmmm
! use dftbp_derivs_perturbxderivs_qmmm_tk
...
#:endif
  ...
  use dftbp_dftbplus_mainio, only : openOutputFile, printBlankLine, printElecConstrHeader,&
      ...
      & writeDetailedOut7, writeDetailedOut8, writeDetailedOut9, writeDetailedXml, writeEigenVectors,&
      & writeEsp, writeFinalDriverstatus, writeHessianout, writehsandstop, writeMdOut1, writeMdOut2,&
...
contains
  ...
  !> Process derivatives of atom charges w.r.t. coordinates of atoms and of external point charges
  subroutine processChargeDerivatives(env, this, nExtChrgWRT, extChrgWRT)
    ...
    call dPsidx(env, this%parallelKS, this%filling, this%eigen, this%eigVecsReal, this%rhoPrim,&
        & this%potential, this%qOutput, this%q0, this%ints%hamiltonian, this%ints%overlap, this%skHamCont,&
    ...
    if (doPsidxQMMM) then
      ...
      if (present(nExtChrgWRT)) then
        ...
        ! create the main structure (list f MM atoms to calculate derivatives with respect to)
      ...
      call dPsidxQMMM(env, this%parallelKS, this%filling, this%eigen, this%eigVecsReal, this%qOutput,&
          & this%q0, this%ints%hamiltonian, this%ints%overlap, this%orb, this%nAtom, this%species,&
          & this%neighbourList, this%nNeighbourSK, this%denseDesc, this%iSparseStart,&
          & this%img2CentCell, this%coord, this%scc, this%maxSccIter, this%sccTol, this%nMixElements,&
          & this%nIneqOrb, this%iEqOrbitals, this%tempElec, this%Ef, this%tFixEf, this%spinW,&
          & this%thirdOrd, this%dftbU, this%iEqBlockDftbu, this%onSiteElements, this%iEqBlockOnSite,&
          & this%hybridXc, this%nNeighbourCam, this%chrgMixerReal, this%taggedWriter,&
          & this%tWriteAutotest, autotestTag, this%tWriteResultsTag, resultsTag, this%tMulliken,&
          & this%dQdXext, this%nExtChrgWRT, this%extChrgWRT)
      call env%globalTimer%stopTimer(globalTimers%perturbMM)
    end if

  ! if (.not. allocated(this%scc)) then
  ...
  !     & this%nNeighbourSK, this%denseDesc, this%iSparseStart, this%img2CentCell, this%coord,&
  !     & this%scc, this%maxSccIter, this%sccTol, this%tempElec, this%thirdOrd,&
  !     & this%pChrgMixer)
  ! call env%globalTimer%stopTimer(globalTimers%perturbQM)
  ! call env%globalTimer%startTimer(globalTimers%perturbMM)
  ...
  ! call env%globalTimer%stopTimer(globalTimers%perturbMM)
    ...
    if (this%tWriteDetailedOut) then
      if (this%nExtChrg > 0) then
        ...
        call writeDetailedOut4a(this%fdDetailedOut%unit, this%dQdX)
      end if
    end if
