...
module dftbp_dftbplus_initprogram
 ...
 use dftbp_dftb_orbitalequiv, only : OrbitalEquiv_merge, OrbitalEquiv_reduce
  ...
  use dftbp_dftbplus_inputdata, only : TBlacsOpts, TControl, THybridXcInp, TInputData,&
      & TParallelOpts
  use dftbp_dftbplus_outputfiles, only : autotestTag, bandOutBase, derivEBandOut, fCharges,&
      & fStopDriver, fStopSCC, hessianOut, mdOut, resultsTag, userOutBase
  ...
  subroutine initOutputFiles(this, env, outFileNameStub)
    ...
    if (this%tWriteBandDat) then
      ...
      if (this%deltaDftb%nDeterminant() == 1) then
        call clearFile(this%bandOut)
      else
        do iDet = 1, this%deltaDftb%nDeterminant()
          call clearFile(this%deltaDftb%determinantName(iDet) // '_' //  this%bandOut)
        end do
      end if
