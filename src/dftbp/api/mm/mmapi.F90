...
module dftbp_mmapi
  ...
  use dftbp_common_globalenv, only : destructGlobalEnv, initGlobalEnv, instanceSafeBuild, stdOut,&
      & withMpi
  ...
  use dftbp_dftbplus_mainapi, only : checkSpeciesNames, doOneTdStep, finalizeTimeProp,&
      & getAtomicMasses, getChargeDerivatives, getCM5Charges, getCutOff, getElStatPotential,&
      & getEnergy, getExtChargeGradients, getGradients, getGrossCharges, getLocalKS, getRefCharges,&
      & getStressTensor, getTdForces, initializeTimeProp, nrOfAtoms, nrOfExtCharges, nrOfKPoints,&
      & nrOfLocalKS, nrOfSpin, setExternalCharges, setExternalEfield, setExternalPotential,&
      & setGeometry, setNeighbourList, setQDepExtPotProxy, setRefCharges, setTdCoordsAndVelos,&
...
contains
  ...
  !> Returns the nr. of external point charges in the system.
  function TDftbPlus_nrOfExtCharges(this) result(nExtCharge)

    !> Instance
    class(TDftbPlus), intent(in) :: this

    !> Nr. of external point charges
    integer :: nExtCharge
