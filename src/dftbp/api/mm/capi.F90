...
module dftbp_capi
  ...
  use dftbp_common_globalenv, only : instanceSafeBuild, stdOut
...
contains
  ...
  subroutine c_DftbPlus_setExternalCharges(handler, nExtCharges, chargeCoords, chargeQs)&
    ...
    ! TODO - possibly change to row major format for consistency with other coordinate arrays
    !      - similarly to above, this would be: real(c_double), intent(in) :: chargeCoords(3,*)
    type(c_ptr), value, intent(in) :: chargeCoords

    !> Magnitudes of external point charges
  ...
  subroutine c_DftbPlus_getChargeDerivatives(handler, dQdX, dQdXext)&
    ...
    ! TODO - possibly change to: real(c_double), intent(in) :: dQdX(3,*,*)
    type(c_ptr), value, intent(in) :: dQdX

    !> Derivatives w.r.t. coords of external point charges, row major format
    ! TODO - possibly change to: real(c_double), intent(in) :: dQdXext(3,*,*)
  ...
  subroutine c_DftbPlus_getChargeDerivativesSelect(handler, dQdX, dQdXext, nExtChrgWRT, extChrgWRT)&
    ...
    !> Number of MM atoms to calculate the derivatives of charges w.r.t. coordinates of those MM atoms
